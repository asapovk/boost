...
class ReturnTypeGenerator {
    ...
    private Rmethods: Array<{ f: string, args: object, debug?: boolean }> = []

    private args: Array<{ args: object, root: string }> = []
    ...
    public genTypes() {
        ...
        for (let m of this.Rmethods) {
            ...
            if (returnTypeString) {
                ...
                for (let t of returnTypeString.tables) {
                    this.pushTableType(t)
                }
            }
            index++
        }
        for (let t of this.tableTypesToBuild) {
            tdsString += '\n\n'
            const tableType = this.opts.schema.getDTSFromTableName(t)
            tdsString += tableType
        }

        fs.writeFileSync(path.join(__dirname, `../../${Store.getConfig().returnPath}`, 'return.d.ts'), tdsString, 'utf-8')
    ...
    public pushRmethod = (args: { f: string, args: object, debug?: boolean }) => {
    ...
    public pushArgs(params: { args: object, root: string }) {
        // this.call()
        this.args.push(params)
    }


    public runMethods(o: object, debug?: boolean) {
        for (let m of this.Rmethods) {
            if (!debug) {
                if (o[m.f]) {
                    o[m.f](m.args)
