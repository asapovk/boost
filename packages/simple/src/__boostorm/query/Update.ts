...
export type UpdateArgs<T extends keyof EntitieTypes._Entities> = {
    ...
    params: Partial<{ [K in keyof GetEntit<EntitieTypes._Entities[T]>]: GetEntit<EntitieTypes._Entities[T]>[K] }>
}
...
class Update extends Query {
    ...
    public update = <T extends keyof EntitieTypes._Entities>(args: UpdateArgs<T>) => {
        let paramsSet: Array<string> = []
        for (let param in args.params) {
            if (args.params[param] !== undefined) {
                paramsSet.push(`${param} = $${this.getParamNumber(args.params[param])}`)
            }
        }
        ...
        return `update ${args.table} \nset ${paramsSet.join(', ')}${this.whereString}`
    }
