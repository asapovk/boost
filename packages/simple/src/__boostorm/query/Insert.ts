...
export type InsertArgs<T extends keyof EntitieTypes._Entities> =
    {
        table: T,
        params: {
            [K in keyof GetCreate<EntitieTypes._Entities[T]>]: GetCreate<EntitieTypes._Entities[T]>[K] | SubSelect
        }
    }
...
class Insert extends Query {
    ...
    public insert = <T extends keyof EntitieTypes._Entities>(args: InsertArgs<T>) => {
        ...
        for (let param in args.params) {
            if (typeof args.params[param] === 'function') {
                ...
                this.paramsArrayNumbers.push(`$${this.getParamNumber(args.params[param])}`)
