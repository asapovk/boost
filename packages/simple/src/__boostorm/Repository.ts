...
export interface RepositoryOpts {
    executor: (query: string, params: Array<unknown>) => Promise<{ rows: Array<unknown>, rowCount: number }>
    schema: Schema
}
...
class Repository {
    ...
    @Root('Insert')
    public async insert<T extends keyof EntitieTypes._Entities>(args: InsertArgs<T>) {
    ...
    @Root('Update')
    public async update<T extends keyof EntitieTypes._Entities>(args: UpdateArgs<T>) {
    ...
    @Root('Fetch')
    public async fetch<T extends keyof EntitieTypes._Entities>(args: FetchArgs<T>) {
    ...
    @Root('Select')
    public async select<T extends keyof EntitieTypes._Entities, L extends keyof EntitieTypes._Entities[T]['entitie'], M extends keyof EntitieTypes._Entities[T]['entitie'], A extends { [key in keyof EntitieTypes._Entities]: EntitieTypes._Entities[key] }>(args: SelectArgs<T, L, M, A>) {
    ...
    @Root('SelectUnsafe')
    public async selectUnsafe<Type extends object, M extends keyof Type, L extends keyof Type, A extends { [key in keyof EntitieTypes._Entities]: EntitieTypes._Entities[key] }>(type: Type, args: SelectUnsafeArgs<Type, M, L, A>) {
        const qs = new SelectUnsafe(this.opts.schema)
        const res = await this.opts.executor(qs.selectUnsafe(type)(args), qs.getParams())
        return res.rows as any
    }

    @Root('SelectWith')
    public async selectWith<T extends { [key: string]: (a: any) => unknown }, K extends keyof T>(args: SelectWithArgs<T, K>) {
